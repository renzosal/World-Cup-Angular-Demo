(function() {
	...
	function datacontext($http, $q, config, $timeout) {
		...
		function getAllGroups() {
			...
			var groupedArray = _(teams).groupBy("group").each(function (value,key,list){
				list[key] = _(value).sortBy('groupPoints').reverse().value();
			}).value();
			deferred.resolve(groupedArray);
		...
		function getTeamByName(name) {
			var deferred = $q.defer();
			var team = _.find(teams, {name: name});
		...
		function getAllPlayers() {
			var promise = $http
				...
				{
					params: {
						fields: 'id,nickname,nationality,age,position,heightCm,weightKg',
						sort: 'lastName',
						limit: '1000'
					}
				})
